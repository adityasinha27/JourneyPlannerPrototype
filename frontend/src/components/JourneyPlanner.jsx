...
const JourneyPlanner = () => {
  ...
  return (
    <div className="bg-white max-w-3xl mx-auto p-6 rounded shadow my-10">
      ...
      <form onSubmit={handlePlanJourney} className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <select value={source} onChange={(e) => setSource(e.target.value)} required className="p-2 border rounded">
          <option value="">Select Source</option>
          {stops.map(stop => (
            <option key={stop._id} value={stop.name}>{stop.name}</option>
          ))}
        </select>
        <select value={destination} onChange={(e) => setDestination(e.target.value)} required className="p-2 border rounded">
      ...
      {loading && <p className="text-center text-gray-500">Searching routes...</p>}

      {!loading && paths.length > 0 && (
        <div>
          <h3 className="font-semibold mb-2 text-lg">Available Paths:</h3>
          {paths.map((path, index) => {
            const summary = getTotalSummary(path);
            return (
              <div key={index} className="mb-4 p-3 border rounded bg-gray-50">
                <p className="text-sm text-blue-700 mb-2">
                  Total Distance: {summary.distance} km | Estimated Time: {summary.duration} mins
                </p>
                  ...
                  {path.map((step, idx) => (
                    <li key={idx} className="mb-1">
                      <div>
                        ...
                        {step.route && (
                          <span className="text-sm text-gray-600 ml-2">(via Route {step.route})</span>
                        )}
                      </div>
                      {step.distanceFromPrevious && step.durationFromPrevious && (
                        <div className="text-sm text-gray-500 ml-2">
                          🧭 {step.distanceFromPrevious} – ⏱️ {step.durationFromPrevious}
                        </div>
                      )}
                    </li>
                  ))}
